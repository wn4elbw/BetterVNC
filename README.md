...
- **图形控制台（run.py）**：顶部导航包含首页、环境、命令行和配置。首页管理服务、端口、
  访问地址及日志；环境页选择 Python 并获取或安装包；配置页用键值对和列表编辑 JSON 设置文件
- **统一运行配置**：主题、传输编码和性能面板开关保存在根目录 `config.json`
...
- **一象限（右上，最大）**：远程桌面，打开即自动连接；右上角提供页面内缩放
- **显示分辨率**：仅允许选择 Windows 显示驱动枚举的模式，宽高均不超过程序启动时检测到的实际屏幕分辨率
- **二象限（左上，细高）**：文件树 ——
